...
1. 初始阶段(第0阶段)，点击确定按钮(fTip01)开始游戏(开始timeline01)
   * 向控制器发送消息，进入第一阶段(黑屏，显示人物模型，播放动画)
   * 播放完毕后场景静止，显示目标标记`DestinationPoint01`
   * 等待玩家走到指定地点后(进入第二阶段)，甲交代工作内容(timeline02)
   * 玩家点击听明白了的按钮(fTip02)

~~空中出现签字板，用手柄获取后进入下一阶段(延迟1秒左右，黑屏)~~

2. 乙和丙去开关柜前，甲对玩家交代事情并离开(timeline03)

   * 甲离开
   * 出现提示框“我先把柜门打开吧”(fTip03)，玩家打开柜门前无下一步操作
   * 出现提示框“打开小室盖板”(fTip04)
   * 玩家触摸小室盖板后，timeline03播放结束
   * 出现提示框，“事故分析和事故回放”(fTip05)
